...
class MyPlugin extends obsidian.Plugin {
    ...
    onload() {
        ...
        */
        ...
        this.addCommand({
            id: 'internal-hyper',
            name: 'callout折叠',
            callback: () => this.内链转为超链接() //我换成callout了
        });
        this.addCommand({
            id: 'hyper-internal',
            name: '[]()转为[[]]',
    ...
    内链转为超链接() {
        this.获取编辑器信息();
        if (!所选文本) return;
        所选文本 = 所选文本.replace(/^(.+)$/mg, "> [!collapse]- $1")
        this.替换所选文本(所选文本);
    };
    // 我换成callout了
        ...
        navigator.clipboard.readText()
            .then(clipText => {
                ...
                if (urlReg.test(tmpText)) {
                    ...
                    clipText = clipText.replace(/(?<=[^a-zA-Z])\s+(?=\r*\n)/g, "")
                    clipText = clipText.replace(/(?<=\w)\s+(?=\r*\n)/g, " ")
                    //去除末尾非字母后的空格，将末尾字母后的多个空格保留一个
                    clipText = clipText.replace(/([^。？！\.\?\!\w])(\r*\n)+/mg, "$1")
                    clipText = clipText.replace(/(\w)(\r*\n)+/mg, "$1 ")

                    clipText = clipText.replace(/(?<=[一-龥])\s*,\s*|(?<=”),\s*/g, "，")
                    clipText = clipText.replace(/(?<=[^a-z0-9\>\]\.])\./g, "。")   //数字后的黑点不转为句号
                    clipText = clipText.replace(/(?<=[^a-z0-9\>\]])\?/g, "？")
                    clipText = clipText.replace(/(?<=[^a-z0-9\>\]])!(?=[^\[])/g, "！")
                    clipText = clipText.replace(/;/g, "；")
                    // clipText = clipText.replace(/(?<=[^:]):|:(?=[^:])/g, "：")
                    clipText = clipText.replace(/(?<=[^a-z0-9\:])\:/g, "：")
                    clipText = clipText.replace(/\((?<=[一-龥])\s*/g, "（")
                    clipText = clipText.replace(/(?<=[一-龥])\s*\)/g, "）")
                    // clipText = clipText.replace(/\(/g,"（")
                    // clipText = clipText.replace(/\)/g,"）")
                    // clipText = clipText.replace(/(?<=[\(（)])[^一-龥]+(?=[\)）])/g,"\($1\)")
                    clipText = clipText.replace(/\{([^{}]*)\}/g, "｛$1｝")
                    clipText = clipText.replace(/\"([^\"]*?)\"/g, "“$1”")
                    clipText = clipText.replace(/\[\d\]|\[\d\d\]|\(\d\)|\（\d\）|°|®|◎|①|②|③|④|©/g, "")
                }
    ...
    修复错误标点() {
        ...
        笔记正文 = 笔记正文.replace(/(?<=\w)\s+(?=\r*\n)/g, " ")
        //去除末尾非字母后的空格，将末尾字母后的多个空格保留一个
        笔记正文 = 笔记正文.replace(/([^。？！\.\?\!\w])(\r*\n)+/mg, "$1")
        笔记正文 = 笔记正文.replace(/(\w)(\r*\n)+/mg, "$1 ")


        笔记正文 = 笔记正文.replace(/(?<=[一-龥])\s*,\s*|(?<=”),\s*/g, "，")
        笔记正文 = 笔记正文.replace(/(?<=[^a-z0-9\>\]\.\ ])\./g, "。")   //数字后的黑点不转为句号
